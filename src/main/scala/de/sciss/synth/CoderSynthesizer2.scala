...
class CodeSynthesizer2 extends Refactoring
...
//                              } else Ident( a.name )
//                           })
//                           if( impliedRate.isDefined ) args0 else (if( expandBin.isDefined ) Select( identRateOrder, "in1" ) else identRate) :: args0
//                        })
...
//                           val args1         = if( hasLastMulti ) args0.dropRight( 1 ) else args0
                           val argsApp       = args0.map( a => {
                              if( a.isGE ) {
//                                 require( !a.multi, "No multi expected here" )
                                 val apply = Apply( Ident( "_" + a.name ), {
                                    if( moreThanOneArgOut ) {
                                       Apply( Select( Ident( "i" ), "%" ), Ident( "_sz_" + a.name ) :: Nil )
                                    } else {
                           ...
                           val (preBody, preArgs, argsApp1) = outputs match {
                              case m: MultiOutputLike =>
                                 val (preBody, tree, argsApp0) = m match {
                                    case fm: FixedMultiOutput => (None, fm.tree, argsApp)
                                    case am @ ArgMultiOutput( a ) => if( a.isGE ) {
                                       require( args0.lastOption == Some( a ))
                                       val chaName    = "_ch_" + a.name
                                       val chaIdent   = Ident( chaName )
                                       val assign     = ValDef( NoMods, chaName, TypeTree( NoType ), argsApp.last )
                                       (Some( assign ), Select( chaIdent, "size" ), argsApp.dropRight( 1 ) :+ chaIdent )
                                    } else (None, am.tree, argsApp)
                                 }
                                 (preBody,
